...
def get_repository_committee(organisation_name, repo_name, committees):
    ...
    for record in content:
        ...
        all_author_data.append(
            Contributor(
                record["author"]["login"], record["author"]["html_url"], record["total"]
            )
        )
...
def get_organisation_repository(
    organisation_name, repo_count, committees, per_page, offset
):
    ...
    if "message" in content.keys() or len(content["items"]) == 0:
        data = {"status": "Failed", "count": 0, "message": f"No data Found "}

        return data

    data = {"status": "Success", "data": [], "count": len(content["items"])}
    ...
    for record in content["items"][low:high]:
        ...
        contributor = get_repository_committee(
        ...
        data["data"].append(
            Repository(record["name"], record["html_url"], record["forks"], contributor)
        )
...
def get_data(org_name, repo_count, committees, per_page, offset):
    ...
    data = get_organisation_repository(
